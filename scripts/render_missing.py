#!/usr/bin/env python3
"""
Render only specific missing frames to fill gaps in render_frames/.
"""
...
# Missing frame numbers (1-indexed)
MISSING_FRAMES = [488, 489, 490, 491, 492]
...
print(f"Rendering {len(MISSING_FRAMES)} missing frames: {MISSING_FRAMES}")
...
for i, anim_f in enumerate(MISSING_FRAMES):
    # path_frames is 0-indexed, anim_f is 1-indexed
    pf = path_frames[anim_f - 1]
    ...
    print(f"  [{i+1}/{len(MISSING_FRAMES)}] Frame {anim_f}: {time_ma:.0f} Ma (geo #{geo_idx})")

print(f"\n✓ Missing frames rendered in {time.time() - render_start:.0f}s")
